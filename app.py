...
def api_explore():
    ...
    try:
        ...
        if q:
            videos = db.execute("""
                SELECT id,title,url,creator,source,caption,tags,uploader_id,
                       COALESCE(views,0) AS views
                FROM videos
                WHERE privacy='public'
                  AND (lower(title) LIKE lower(?) OR lower(COALESCE(caption,'')) LIKE lower(?)
                       OR lower(COALESCE(tags,'')) LIKE lower(?) OR lower(creator) LIKE lower(?))
                ORDER BY COALESCE(views,0) DESC, id DESC LIMIT 24
