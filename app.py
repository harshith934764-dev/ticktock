...
def api_explore():
    """Explore/discovery feed: trending creators, public Reels, and hashtags."""
    q = str(request.args.get("q", "")).strip()[:80]
    db = get_db()
    try:
        # Views are stored in video_views, not videos.views.
        users = db.execute("""
            ...
            videos = db.execute("""
                SELECT v.id, v.title, v.url, v.creator, v.source, v.caption, v.tags,
                       ...
                       (SELECT COUNT(*) FROM video_views vv
                        WHERE CAST(vv.video_id AS TEXT)=CAST(v.id AS TEXT)) AS views
                FROM videos v
                WHERE v.privacy='public'
                  AND (lower(v.title) LIKE lower(?)
                       OR lower(COALESCE(v.caption,'')) LIKE lower(?)
                       OR lower(COALESCE(v.tags,'')) LIKE lower(?)
                       OR lower(v.creator) LIKE lower(?))
        ...
        rows = db.execute("""
            ...
            WHERE privacy='public' AND tags IS NOT NULL AND tags != ''
            LIMIT 500
        """).fetchall()
        ...
        print("Explore API error:", repr(error))
        return jsonify({
            "success": False,
            "users": [],
            "videos": [],
            "hashtags": [],
            "message": "Explore could not be loaded right now."
        }), 500
    finally:
        db.close()
