...
# Admin access is kept outside the users table so the administrator password
# is never stored in the database or committed to GitHub. Configure these
# values as Render Environment Variables.
...
def production_request_guards():
    ...
    if rule is None:
        prefix_rules = (
            ...
            ("/api/like/", (120, 600)),
            ("/api/follow/", (60, 600)),
            ("/api/friend-requests", (30, 600)),
            ("/api/report", (20, 3600)),
            ("/api/view/", (300, 600)),
            ("/api/gifts/send", (60, 600)),
            ("/api/diamonds/create-order", (20, 3600)),
        )
    ...
    if rule and request.method == "POST":
        limit, window = rule
        key = (_client_ip(), request.path.split("/")[:4][0] if request.path.startswith("/api/") else request.path)
...
def production_security_headers(response):
    ...
    if request.path in {"/login", "/register", "/forgot-password", "/reset-password", "/admin/login"}:
        response.headers["Cache-Control"] = "no-store"
    elif request.path in {"/healthz", "/robots.txt", "/privacy", "/terms", "/about"}:
...
SUPABASE_STORAGE_KEY = SUPABASE_SECRET_KEY or SUPABASE_SERVICE_ROLE_KEY
...
def storage_configured():
    return bool(SUPABASE_URL and SUPABASE_STORAGE_KEY)
...
def ensure_storage_bucket(bucket):
    if not storage_configured():
        print("Tick Tock Storage: SUPABASE_URL / SUPABASE_SECRET_KEY not configured yet.")
        return False
    headers = {"Authorization": f"Bearer {SUPABASE_STORAGE_KEY}", "apikey": SUPABASE_STORAGE_KEY}
    try:
...
def upload_to_storage(file_storage, path, content_type):
    if not storage_configured():
        raise RuntimeError("Cloud media storage is not configured. Add SUPABASE_SECRET_KEY in Render.")
    bucket = storage_bucket_for_path(path)
    if not ensure_storage_bucket(bucket):
        raise RuntimeError(f"Supabase Storage bucket '{bucket}' is not ready.")
    file_storage.stream.seek(0)
    # Keep the browser's video decoder and HTTP range requests happy.
    # Some mobile browsers behave poorly when an uploaded MP4 is labelled as
    # application/octet-stream, so infer the MIME type from the file extension.
...
def _init_sqlite():
    ...
    # AI bot metadata (safe migration for existing databases).
    add_column_if_missing(db, "users", "is_ai_bot", "INTEGER DEFAULT 0")
    add_column_if_missing(db, "users", "ai_bot_role", "TEXT")
    ...
    db.execute("""CREATE TABLE IF NOT EXISTS reports (
        ...
        reported_user_id INTEGER, reason TEXT NOT NULL, status TEXT DEFAULT 'open',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )""")
    add_column_if_missing(db, "reports", "status", "TEXT DEFAULT 'open'")
    for c, d in [
...
def _init_postgres():
    ...
    statements = [
        ...
        """ALTER TABLE users ADD COLUMN IF NOT EXISTS is_ai_bot INTEGER DEFAULT 0""",
        """ALTER TABLE users ADD COLUMN IF NOT EXISTS ai_bot_role TEXT""",
        ...
        """CREATE TABLE IF NOT EXISTS reports (
            ...
            reported_user_id BIGINT, reason TEXT NOT NULL, status TEXT DEFAULT 'open',
            created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
        )""",
    ]
...
init_db()
# Storage buckets are optional at startup until Supabase Storage credentials are added.
...
AI_BOTS = [
    ("ticktock_ai_guide", "Tick Tock AI Guide", "content-guide"),
    ("ticktock_ai_trends", "Tick Tock AI Trends", "trend-guide"),
    ("ticktock_ai_fun", "Tick Tock AI Fun", "friendly-commenter"),
]


def ensure_ai_bots(db):
    """Create clearly labelled AI accounts once; never pretend they are humans."""
    for username, display_name, role in AI_BOTS:
        row = db.execute("SELECT id FROM users WHERE username=?", (username,)).fetchone()
        if row:
            db.execute("UPDATE users SET is_ai_bot=1, ai_bot_role=?, display_name=?, profile_complete=1 WHERE id=?",
        ...
        db.execute("""
            INSERT INTO users
            (username,password,uid,friend_uid,bio,profile_complete,display_name,is_ai_bot,ai_bot_role)
            VALUES (?,?,?,?,?,?,?,?,?)
        """, (username, secrets.token_hex(24), uid, friend_uid,
              "AI account • clearly labelled 🤖 • helps explain and discover Shorts",
              1, display_name, 1, role))
    db.commit()


def seed_ai_likes(db, video_ids):
    """Give each eligible Short a small, transparent AI-community signal.
    This is intentionally capped to avoid artificial-looking engagement spikes.
    """
    ensure_ai_bots(db)
    bot_rows = db.execute("SELECT id FROM users WHERE is_ai_bot=1 ORDER BY id LIMIT 3").fetchall()
    if not bot_rows:
        return
    for raw_id in list(dict.fromkeys(str(v) for v in video_ids if v))[:60]:
        for bot in bot_rows:
            try:
                db.execute("INSERT INTO likes(user_id,video_id) VALUES(?,?) ON CONFLICT(user_id,video_id) DO NOTHING",
                           (bot["id"], raw_id))
            except Exception:
                # SQLite versions without ON CONFLICT syntax compatibility fall back safely.
                try:
                    exists = db.execute("SELECT id FROM likes WHERE user_id=? AND video_id=?",
                                        (bot["id"], raw_id)).fetchone()
                    if not exists:
                        db.execute("INSERT INTO likes(user_id,video_id) VALUES(?,?)", (bot["id"], raw_id))
                except Exception:
                    pass
    db.commit()


def ai_content_summary(item):
    """Fast local summary; no external AI call required for every feed item."""
    text = " ".join(str(item.get(k) or "") for k in ("title", "caption", "tags", "creator"))
    low = text.lower()
    topics = [
        ("cricket", "cricket"), ("football", "football"), ("soccer", "football"),
        ("gaming", "gaming"), ("game", "gaming"), ("dance", "dance"),
        ("music", "music"), ("food", "food"), ("travel", "travel"),
        ("nature", "nature"), ("animal", "animals"), ("car", "cars"),
        ("bike", "bikes"), ("technology", "technology"), ("tech", "technology"),
        ("fitness", "fitness"), ("comedy", "comedy"), ("funny", "comedy"),
    ]
    topic = next((name for key, name in topics if key in low), "short-form entertainment")
    return f"This Short is about {topic}. Watch the video for the main moment, then use Ask AI for more context."


@app.route("/api/ai/community")
def ai_community_info():
    if not logged_in():
        return jsonify({"success": False, "login_required": True}), 401
    db = get_db()
    ensure_ai_bots(db)
    bots = db.execute("SELECT username,display_name,bio,uid FROM users WHERE is_ai_bot=1 ORDER BY id").fetchall()
    db.close()
    return jsonify({"success": True, "bots": [dict(x) for x in bots]})


@app.route("/api/ai/explain")
def ai_explain_short():
    if not logged_in():
        return jsonify({"success": False, "login_required": True}), 401
    payload = {"title": request.args.get("title", ""), "caption": request.args.get("caption", ""),
               "tags": request.args.get("tags", ""), "creator": request.args.get("creator", "")}
    return jsonify({"success": True, "summary": ai_content_summary(payload), "label": "AI explanation"})
...
def like_status():
    if not logged_in():
        return jsonify({}), 401
    ids = [x.strip() for x in request.args.get("ids", "").split(",") if x.strip()][:60]
    if not ids:
        return jsonify({})
    db = get_db()
    me = session["user_id"]
    out = {}
    for vid in ids:
        count = db.execute("SELECT COUNT(*) AS c FROM likes WHERE video_id=?", (vid,)).fetchone()["c"]
        liked = db.execute("SELECT 1 FROM likes WHERE video_id=? AND user_id=?", (vid, me)).fetchone() is not None
        ai_count = db.execute("""SELECT COUNT(*) AS c FROM likes l JOIN users u ON u.id=l.user_id
                                 WHERE l.video_id=? AND u.is_ai_bot=1""", (vid,)).fetchone()["c"]
        out[vid] = {"liked": liked, "count": count, "ai_count": ai_count}
    db.close()
    return jsonify(out)
...
# =========================================================
# PRODUCT / PLATFORM ROUTES
# =========================================================

@app.route("/healthz")
def healthz():
    """Small health endpoint for Render and uptime checks."""
    try:
        db = get_db()
        db.execute("SELECT 1").fetchone()
        db.close()
        return jsonify({"ok": True, "service": "ticktock"})
    except Exception:
        return jsonify({"ok": False, "service": "ticktock"}), 503
...
def api_explore():
    """Explore/discovery feed: trending tags, creators, and recent/public Reels."""
    q = str(request.args.get("q", "")).strip()[:80]
    db = get_db()
    try:
        users = db.execute("""
            SELECT u.id, u.username, u.uid, u.friend_uid, u.gender, u.photo, u.bio,
                   (SELECT COUNT(*) FROM follows f WHERE f.following_id=u.id) AS followers_count
            FROM users u
            WHERE u.profile_complete=1
        ...
        if q:
            videos = db.execute("""
                SELECT v.id,v.title,v.url,v.creator,v.source,v.caption,v.tags,v.uploader_id,
                ...
                LEFT JOIN (
                    ...
                    FROM video_views
                    GROUP BY video_id
                ) vc ON CAST(vc.video_id AS TEXT)=CAST(v.id AS TEXT)
                WHERE v.privacy='public'
                  AND (lower(v.title) LIKE lower(?) OR lower(COALESCE(v.caption,'')) LIKE lower(?)
                       OR lower(COALESCE(v.tags,'')) LIKE lower(?) OR lower(v.creator) LIKE lower(?))
                ORDER BY COALESCE(vc.views,0) DESC, v.id DESC LIMIT 24
        ...
        # Hashtags are derived from public video tags. This is intentionally simple
        # and works with the existing SQLite schema.
        rows = db.execute("SELECT tags FROM videos WHERE privacy='public' AND tags IS NOT NULL AND tags != '' LIMIT 500").fetchall()
        counts = {}
        for row in rows:
            raw = str(row["tags"] or "")
            for token in re.split(r"[\s,]+", raw):
                token = token.strip().lstrip("#").lower()
                if not token or len(token) > 40 or not re.match(r"^[a-z0-9_]+$", token):
                    continue
                counts[token] = counts.get(token, 0) + 1
        hashtags = [{"tag":k,"count":v} for k,v in sorted(counts.items(), key=lambda x:(-x[1],x[0]))[:20]]
...
VIDEO_CACHE_TTL = 2 * 60 * 60

def get_pixabay_videos():

    global VIDEO_CACHE, VIDEO_CACHE_TIME

    # Long cache = fast page loads and far fewer external API calls.
    ...
    def fetch_query(query):
        try:
            ...
            for item in data.get("hits", []):
                duration = item.get("duration", 9999)
                if duration > 30:
                    continue
                files = item.get("videos", {})
                selected = None
                # Smaller files start faster on mobile/slow connections.
            ...
            print("Pixabay query error:", query, error)
            return []

    result = []
    # Run external searches in parallel so 4 slow API calls do not become 4x slower.
    with ThreadPoolExecutor(max_workers=min(4, len(queries))) as pool:
        futures = [pool.submit(fetch_query, q) for q in queries]
        for future in as_completed(futures):
            result.extend(future.result())

    unique = {video["id"]: video for video in result}
    VIDEO_CACHE = list(unique.values())[:32]
...
def api_videos():
    ...
    try:
        db = get_db(); seed_ai_likes(db, [v.get("id") for v in videos]); db.close()
    except Exception as error:
        print("AI like seed warning:", error)
    resp = jsonify(videos)
...
def upload():
    ...
    try:
        media_url = upload_to_storage(file, storage_path, content_type or mimetypes.guess_type(filename)[0] or "video/mp4")
    except Exception as error:
...
def api_uploads():
    ...
    try:
        seed_ai_likes(db, ["upload_" + str(row["id"]) for row in rows])
    except Exception as error:
        print("AI upload like seed warning:", error)
    db.close()
...
def admin_login():
    ...
    if not admin_configured():
        return render_template(
            "admin_login.html",
            error="Admin access is not configured. Set ADMIN_USERNAME and ADMIN_PASSWORD in your server environment."
        ), 503
    ...
    if request.method == "POST":
        ...
        if hmac.compare_digest(username, ADMIN_USERNAME) and hmac.compare_digest(password, ADMIN_PASSWORD):
            ...
            return redirect(url_for("admin_dashboard"))
        return render_template("admin_login.html", error="Invalid admin credentials."), 401
...
def admin_dashboard():
    ...
    db = get_db()
    try:
        users = db.execute("SELECT id, username, display_name, email_verified, profile_complete, is_ai_bot, created_at FROM users ORDER BY id DESC LIMIT 100").fetchall()
        videos = db.execute("SELECT id, title, creator, url, privacy, created_at FROM videos ORDER BY id DESC LIMIT 100").fetchall()
        reports = db.execute("""
            SELECT r.id, r.reason, r.video_id, r.reported_user_id, r.created_at,
                   r.reporter_id, r.status,
        ...
        stats = {
            ...
            "videos": db.execute("SELECT COUNT(*) AS n FROM videos").fetchone()["n"],
            "comments": db.execute("SELECT COUNT(*) AS n FROM comments").fetchone()["n"],
            "reports": db.execute("SELECT COUNT(*) AS n FROM reports").fetchone()["n"],
        }
        return render_template("admin.html", users=users, videos=videos, reports=reports, stats=stats)
    finally:
        db.close()
...
def admin_report_status(report_id):
    ...
    if guard:
        return guard
    status = request.form.get("status", "reviewed").strip().lower()
    if status not in {"open", "reviewed", "dismissed"}:
        return jsonify({"success": False, "message": "Invalid report status."}), 400
    db = get_db()
    try:
        if not column_exists(db, "reports", "status"):
            db.execute("ALTER TABLE reports ADD COLUMN status TEXT DEFAULT 'open'")
...
def admin_delete_video(video_id):
    ...
    try:
        ...
        url = video["url"] if "url" in video.keys() else ""
        db.execute("DELETE FROM likes WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM comments WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM shares WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM bookmarks WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM video_views WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM notifications WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM gifts WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM reports WHERE video_id=?", (str(video_id),))
        db.execute("DELETE FROM videos WHERE id=?", (video_id,))
        db.commit()
        # Best-effort cleanup for Tick Tock-managed local/storage paths.
        if url and (str(url).startswith(SUPABASE_URL) or str(url).startswith("/uploads/") or str(url).startswith("uploads/")):
            path = str(url).split("/storage/v1/object/public/", 1)[-1] if "/storage/v1/object/public/" in str(url) else ""
            if path:
