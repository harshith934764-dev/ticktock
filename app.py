...
def _trusted_user(email):
    ...
    db = get_db()
    user = db.execute(
        "SELECT * FROM users WHERE username=? AND trusted_token_hash=? AND trusted_token_expires>?",
...
def reset_password():
    ...
    if not valid_password(new_password):
        return render_template("reset_password.html", email=email, error="Password needs 8+ characters, uppercase, lowercase, number and special character.")
