...
export default function Home(){
 ...
 {error&&<div className="error">{error}</div>}<section className="video-list">{videos.map(v=><VideoCard key={v.id} video={v}/>)}{!error&&!videos.length&&<p>No videos available yet.</p>}</section><AIAssistant /></main>;
}
